...
add a (Set s) = if member a (Set s) then Set s else Set (sort (a:s))
...
data State = Start | Egged | Floured | Sugared | FlouredAndSugared | Mixed | Error | Finished
  deriving (Eq,Show)
...
average (a :| []) = a
average (a :| as) = (a + sum as) / fromIntegral (length as + 1)
