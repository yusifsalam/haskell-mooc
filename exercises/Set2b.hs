...
oddFactorial 1 = 1
oddFactorial x = if odd x then x * (oddFactorial (x-2)) else oddFactorial (x-1)
...
countdown' :: Integer -> String
countdown' n = if n == 1 then show n ++ "..." else show n ++ "... " ++ countdown' (n-1) 
...
smallestDivisor' n x = if mod n x == 0 then x else smallestDivisor' n (x+1)
