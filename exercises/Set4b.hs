...
countHelper current prev = case current of
    Nothing -> prev + 1
    Just _ -> prev
...
slHelper current prev = (fst prev + current, snd prev + 1)
...
concatStart = []
concatHelper :: [a] -> [a] -> [a]
concatHelper current prev = current ++ prev
...
headHelper :: p -> Maybe p -> Maybe p
headHelper current Nothing = Just current
headHelper current (Just prev) = Just current
...
lastHelper current (Just prev) = Just prev
