...
count a f = length $ filter (==a) (toList f)
...
instance Foldable List where
  ...
  foldr f initial (LNode a b) = f a (foldr f initial b)
...
instance Foldable TwoList where
  ...
  foldr f initial (TwoNode a b c) = f a (f b (foldr f initial c))
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mconcat []
sumTree (Node a b c) = sumTree b <> mconcat [a] <> sumTree c 
