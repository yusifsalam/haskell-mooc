...
myCompare a b = if length a == length b then compare (head b) (head a) else compare (length a) (length b)

longest :: Ord a => [[a]] -> [a]
longest = maximumBy myCompare
...
incrementKey key xs = [if key == k then (k, v+1) else (k,v) | (k,v) <- xs]
...
transfer from to amount bank
    | Map.member from bank && Map.member to bank && amount >= 0 = case Map.lookup from bank of 
        Just value -> if value >= amount then Map.adjust (amount +) to (Map.adjust (-amount +) from bank) else bank
        Nothing -> bank
    | otherwise = bank
