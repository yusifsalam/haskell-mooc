...
tick :: UpDown -> UpDown
tick (Increasing a) = Increasing (a+1)
tick (Decreasing a) = Decreasing (a-1)
...
toNat x
  | x == 0 = Just Zero
  | x < 0 = Nothing 
  | x == 1 = Just (PlusOne Zero)
  | otherwise = case toNat (x-1) of
      Nothing -> Nothing 
      Just Zero -> Just Zero
      Just i -> Just (PlusOne i)
...
prettyPrint End = ""
prettyPrint (O x) = prettyPrint x ++ show 0
prettyPrint (I x) = prettyPrint x ++ show 1 

fromBin :: Bin -> Int
fromBin b = foldl' (\acc x -> acc * 2 + digitToInt x) 0 (prettyPrint b)
