...
allValues condition (Node a b c)
  | condition a = allValues condition b && allValues condition c
  | otherwise = False
...
isOrdered (Node a l Empty) = allValues (<a) l && isOrdered l
isOrdered (Node a Empty r) = allValues (>a) r && isOrdered r
isOrdered (Node a l r) = isOrdered (Node a l Empty) && isOrdered (Node a Empty r)
...
set _ _ Empty = Empty
set [] value (Node a l r) = Node value l r
set (step:path) val (Node a l r) = case walk (step:path) (Node a l r) of
   Nothing -> Node a l r
   Just _ -> case step of
     StepL -> Node a (set path val l) r
     StepR -> Node a l (set path val r)
